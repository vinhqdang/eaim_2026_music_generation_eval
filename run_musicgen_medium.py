"""
Generate 300 audio samples using MusicGen-medium for comparison.
"""
import torch
...
def generate_musicgen_medium(num_prompts=100, num_seeds=3):
    """Generate full MusicGen-medium dataset."""
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {device}")

    print("="*60)
    print(f"MUSICGEN-MEDIUM GENERATION: {num_prompts} prompts × {num_seeds} seeds")
    print("="*60)
    ...
    # Use safetensors to bypass torch.load CVE-2025-32434 issue
    ...
    output_dir = Path("runs/artifacts/wav/musicgen_medium")
    ...
    with tqdm(total=total_samples, desc="MusicGen-Medium Generation") as pbar:
    ...
    print(f"\n{'='*60}")
    print(f"MUSICGEN-MEDIUM COMPLETE")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    print("="*60)
    print("MUSICGEN-MEDIUM GENERATION FOR MODEL COMPARISON")
    print("="*60)
    print("\nTarget: 300 audio samples")
    print("Estimated time: 50-60 minutes on GPU")
    print()

    results = generate_musicgen_medium(num_prompts=100, num_seeds=3)
    ...
    print("\nNext steps:")
    print("1. Run: python3 compute_full_metrics.py")
    print("2. Run: python3 generate_full_analysis.py")
